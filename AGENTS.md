...
| List transitions | GET | `/rest/api/2/issue/{key}/transitions` — returns available workflow transitions |
...
- `--show-tickets` has search/filter via a `textinput.Model` (`histSearch`). Press `/` to focus it; typing narrows the list by title, JiraKey, ticket type, or parent key (`histFiltered()` helper). Esc while non-empty clears the filter; Esc while empty (or Enter) blurs and resumes navigation. Navigation keys (↑↓/jk/PgUp/PgDn) work regardless of focus.
- `--show-tickets` supports bulk status refresh via `R` key: `cmdRefreshAllStatuses` captures `idMap` (JiraKey→ID) and all keys, batches them into groups of 50 using `key in (...)` JQL via `searchIssues`, writes results to DB via `updateTicketStatus`, and returns `bulkStatusMsg`. The handler updates `m.histRecords` in-memory and sets `histStatusRefreshMsg` (shown in footer until next keypress). `histStatusRefreshing` prevents double-firing.
- `--show-tickets` supports status transitions via `t` key: `cmdGetTransitions` calls `GetIssue` (to get current status) then `GetTransitions`, returning `transitionsLoadedMsg` which sets `histTransitionActive=true`. The picker renders inline below the list. Enter fires `cmdTransitionIssue` which calls `TransitionIssue` then `updateTicketStatus` (local DB) and returns `transitionAppliedMsg`. Both commands build a client from config if `m.client` is nil (same pattern as `cmdDeleteJiraIssue`). Status is shown as a `[status]` tag on each row when non-empty. Schema migration 2 added the `status` column.
- `--show-tickets` supports sorting via `histSortField` ("date"|"key"|"title"|"type") and `histSortAsc`. `s` cycles the field (resetting to the natural direction for each: date→desc, rest→asc); `S` toggles direction. Sorting runs inside `histFiltered()` before filtering, using `sort.SliceStable`. The current sort is shown in the page-info subtitle (e.g. `Date↓`).
- `--show-tickets` has two delete modes: `d` deletes the local DB record only (synchronous, no network); `D` (shift+d) calls `DeleteIssue` (async `tea.Cmd`), which removes the issue from Jira then removes the local record on success. Both modes operate on `histFiltered()[histCursor]` so they work correctly when a filter is active. `histConfirmDelete` / `histConfirmDeleteJira` gate the respective confirmation prompts; `histJiraDeleteErr` holds any Jira API error and is displayed in the footer until the next keypress. `cmdDeleteJiraIssue` fires the async delete and returns `jiraDeleteMsg`.
