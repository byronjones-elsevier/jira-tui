...
| **Medium** | `main.go` | `textarea.New()` creates styles using `lipgloss.AdaptiveColor`, which fires a one-time OSC 11 terminal background-color query (`\033]11;?\007`) via `sync.Once` on first render. The Epic Setup screen is the first screen to contain a textarea; by then bubbletea owns stdin, so the terminal response arrives as keystrokes and flows into the focused title textinput (visible as `]11;rgb:0000/0000/0000[50;1R`). **Fixed in code** (`lipgloss.SetHasDarkBackground(true)` called in `main()` before `tea.NewProgram()` — sets `explicitBackgroundColor`, preventing the query from ever firing). |
| **Low** | `model.go:handleDoneKey` | `e` to export CSV called `exportResultsCSV()` but discarded the error and returned the model unchanged — no state update, no view re-render, no user feedback. The file was written but looked like nothing happened. **Fixed in code** (`exportedPath` and `exportErr` fields added to model; `handleDoneKey` stores the result; `viewDone` renders `✓ Exported: …` on success or a red error line on failure). |
